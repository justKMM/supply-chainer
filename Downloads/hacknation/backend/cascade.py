...
Each step produces AI-reasoned decisions and logs messages through the registry.
"""

from __future__ import annotations
import asyncio, random
from datetime import datetime, timedelta
from backend.config import FERRARI_ASSEMBLY_PLANT, BUDGET_CEILING_EUR, TRUST_THRESHOLD, MIN_ESG_SCORE
from backend.schemas import LiveMessage, make_id
from backend.registry import registry
from backend.agents import (
    ai_reason, ai_decompose_bom, create_seed_agents,
    CATEGORY_AGENT_MAP, client,
)
from backend.pubsub import event_bus, DisruptionCategory, SupplyChainEvent
from backend.reputation import reputation_ledger, TransactionRecord
from backend.intelligence import generate_intelligence_signals
...
def _emit(from_id, from_label, to_id, to_label, msg_type, summary, detail="", color="#2196F3", icon="info"):
    msg = LiveMessage(
...
async def run_cascade(intent: str, budget_eur: float = BUDGET_CEILING_EUR) -> dict:
    ...
    report = {
        ...
        "discovery_results": {"agents_discovered": 0, "agents_qualified": 0, "agents_disqualified": 0,
                              "disqualification_reasons": [], "discovery_paths": []},
        "negotiations": [],
        ...
        "execution_plan": {"total_cost_eur": 0, "cost_breakdown": {"components_eur": 0, "logistics_eur": 0, "insurance_eur": 0, "compliance_fees_eur": 0},
                           "timeline": {}, "suppliers_engaged": 0, "purchase_orders_issued": 0, "risk_assessment": {"overall_risk": "low", "risks": []}},
        "disruptions_handled": [],
    ...
    try:
        ...
        _emit("system", "System", "registry", "Agent Registry", "system",
              "Initializing agent network...", "Registering all supply chain agents", "#9E9E9E", "settings")
        ...
        for agent in seed_agents:
            ...
            event_bus.subscribe(agent.agent_id, agent.name, agent.role,
                                regions=list(set(regions)), product_categories=list(set(product_cats)))

        _emit("system", "System", "registry", "Agent Registry", "pubsub_init",
              f"Event bus initialized: {len(event_bus.list_subscriptions())} agents subscribed",
              "Role-based disruption subscriptions configured", "#00BCD4", "broadcast")

        await asyncio.sleep(0.3)
        cascade_state["progress"] = 5

        # ── Step 1: Decompose Intent into BOM ────────────────────────────
        _emit("ferrari-procurement-01", "Ferrari Procurement", "system", "System", "intent_decomposition",
              f"Decomposing intent: {intent}", "Using AI to identify required component categories", "#DC143C", "brain")

        reasoning = await ai_reason("Ferrari Procurement AI", "procurement_agent",
        ...
        _emit("ferrari-procurement-01", "Ferrari Procurement", "system", "System", "bom_complete",
              f"BOM decomposed: {len(bom)} categories, {total_parts} unique parts",
              ", ".join(c["category"] for c in bom), "#DC143C", "checklist")
        ...
        for cat_info in bom:
            ...
            _emit("ferrari-procurement-01", "Ferrari Procurement", "registry", "Agent Registry", "discovery",
            ...
            for c in candidates:
                if c.trust and c.trust.trust_score >= TRUST_THRESHOLD:
                    ...
                    disqualified.append({"agent_id": c.agent_id, "reason": f"Trust score {c.trust.trust_score if c.trust else 0} below threshold {TRUST_THRESHOLD}"})
            ...
            if final:
                ...
                _emit("registry", "Agent Registry", "ferrari-procurement-01", "Ferrari Procurement", "discovery_result",
        ...
        for cat, agent in qualified_agents.items():
            ...
            _emit("ferrari-procurement-01", "Ferrari Procurement", agent.agent_id, agent.name, "request_quote",
            ...
            # Supplier responds
            reasoning = await ai_reason(agent.name, agent.role,
            ...
            _emit(agent.agent_id, agent.name, "ferrari-procurement-01", "Ferrari Procurement", "quote_response",
        ...
        for cat, quote in quotes.items():
            ...
            _emit("ferrari-procurement-01", "Ferrari Procurement", agent.agent_id, agent.name, "negotiate",
                  f"Counter offer: EUR {offer_price:,.0f}/unit ({discount_ask:.1f}% discount)",
                  f"Volume commitment justifies discount", "#FF9800", "negotiate")
            ...
            _emit(agent.agent_id, agent.name, "ferrari-procurement-01", "Ferrari Procurement", "negotiate_response",
                  f"Counter: EUR {counter_price:,.0f}/unit ({supplier_discount:.1f}% off list)",
                  f"Material costs limit flexibility", "#FF9800", "negotiate")
            ...
            _emit("ferrari-procurement-01", "Ferrari Procurement", agent.agent_id, agent.name, "negotiate",
                  ...
                  f"Deal within budget ceiling", "#4CAF50", "handshake")

            negotiation_log = [
                {"round": 1, "from": "ferrari-procurement-01", "action": "counter_offer", "value_eur": offer_price,
                 "reasoning": f"Volume commitment warrants {discount_ask:.1f}% discount"},
                {"round": 2, "from": agent.agent_id, "action": "counter_offer", "value_eur": counter_price,
                 "reasoning": f"Material costs limit discount to {supplier_discount:.1f}%"},
                {"round": 3, "from": "ferrari-procurement-01", "action": "accept", "value_eur": final_price,
                 "reasoning": f"Split difference, within budget ceiling"},
            ]
        ...
        compliance_agent = registry.get("eu-compliance-agent-01")
        checks_passed = 0
        checks_flagged = 0
        flags = []
        ...
        for cat, order in final_orders.items():
            agent = order["agent"]
            _emit("ferrari-procurement-01", "Ferrari Procurement", "eu-compliance-agent-01", "EU Compliance Validator", "compliance_check",
                  f"Requesting compliance check for {agent.name}",
                  f"Checks: certification, sanctions, ESG, regulation", "#FF9800", "shield")
            ...
            if esg_marginal:
                flags.append({"agent_id": agent.agent_id, "check": "esg_threshold",
            ...
            _emit("eu-compliance-agent-01", "EU Compliance Validator", "ferrari-procurement-01", "Ferrari Procurement", "compliance_result",
                  ...
                  "#4CAF50" if all_pass else "#F44336", "check" if all_pass else "warning")
        ...
        for cat, order in final_orders.items():
            ...
            _emit("ferrari-procurement-01", "Ferrari Procurement", agent.agent_id, agent.name, "purchase_order",
            ...
            _emit(agent.agent_id, agent.name, "ferrari-procurement-01", "Ferrari Procurement", "order_confirmation",
        ...
        # ── Step 7: Logistics Planning ───────────────────────────────────
        logistics_agent = registry.get("dhl-logistics-01")
        shipments = []
        ...
        for cat, order in final_orders.items():
            ...
            _emit(agent.agent_id, agent.name, "dhl-logistics-01", "DHL Supply Chain", "logistics_request",
            ...
            _emit("dhl-logistics-01", "DHL Supply Chain", agent.agent_id, agent.name, "logistics_proposal",
        ...
        report["logistics_plan"] = {
            "total_shipments": len(shipments),
            "total_logistics_cost_eur": round(total_logistics_cost, 2),
            "shipments": shipments,
            "critical_path_days": max_lead_days,
            "bottleneck": f"{bottleneck_product} — {max_lead_days} day lead time from {bottleneck_supplier}" if max_lead_days > 0 else "None",
        }

        cascade_state["progress"] = 90

        # ── Step 8: Disruption Simulation ────────────────────────────────
        disrupted_agent = registry.get("brembo-brake-supplier-01")
        _emit("brembo-brake-supplier-01", "Brembo S.p.A.", "ferrari-procurement-01", "Ferrari Procurement", "disruption_alert",
        ...
        _emit("ferrari-procurement-01", "Ferrari Procurement", "registry", "Agent Registry", "discovery",
              ...
              "Triggered by Brembo disruption alert", "#F44336", "search")

        _emit("ferrari-procurement-01", "Ferrari Procurement", "system", "System", "resolution",
        ...
        _emit("system", "System", "reputation-ledger", "Reputation Ledger", "reputation_init",
              "Recording transaction attestations for all suppliers",
              "Building verifiable reputation scores from actual transaction data", "#00BCD4", "ledger")
        ...
        for cat, order in final_orders.items():
            ...
            _emit("reputation-ledger", "Reputation Ledger", agent.agent_id, agent.name, "attestation",
                  f"{agent.name}: {len(attestations)} attestations recorded, composite score {score.composite_score:.3f}",
                  f"Delivery: {score.delivery_score:.2f} | Quality: {score.quality_score:.2f} | Pricing: {score.pricing_score:.2f}",
                  "#00BCD4", "certificate")

        report["reputation_summary"] = reputation_ledger.get_summary()

        cascade_state["progress"] = 94

        # ── Step 10: Intelligence Feed — Real-world signals ──────────────
        _emit("intelligence-feed", "Intelligence Feed", "network", "Agent Network", "intel_init",
              "Scanning external intelligence sources...",
              "Weather, regulatory, commodity, port, and recall databases", "#E91E63", "satellite")
        ...
        _emit("intelligence-feed", "Intelligence Feed", "network", "Agent Network", "intel_complete",
        ...
        total_cost = total_component_cost + total_logistics_cost + insurance_cost + compliance_fees
        ...
        report["execution_plan"] = {
            ...
            "cost_breakdown": {
                ...
                "logistics_eur": round(total_logistics_cost, 2),
        ...
        ferrari = registry.get("ferrari-procurement-01")
        nodes.append({
            "id": "ferrari-procurement-01", "label": "Ferrari Procurement", "role": "procurement_agent",
            "color": "#DC143C", "location": {"lat": 44.5294, "lon": 10.8633, "city": "Maranello"},
            "trust_score": None, "status": "active", "size": 45,
        })
        ...
        for cat, order in final_orders.items():
            ...
            if agent.agent_id not in node_ids_added:
                ...
                nodes.append({
                    "id": agent.agent_id, "label": agent.name, "role": agent.role,
                    "color": color_map.get(agent.role, "#2196F3"), "location": loc,
            ...
            edges.append({
                "from": "ferrari-procurement-01", "to": agent.agent_id, "type": "procurement",
                "label": order.get("po_number", ""), "value_eur": round(order["final_price"] * order["quantity"], 2),
                "message_count": 6, "status": "confirmed",
            })

        # Add logistics node
        if "dhl-logistics-01" not in node_ids_added:
            nodes.append({
                "id": "dhl-logistics-01", "label": "DHL Supply Chain", "role": "logistics_provider",
                "color": "#9C27B0", "location": {"lat": 45.4654, "lon": 9.1859, "city": "Milan"},
                "trust_score": 0.92, "status": "active", "size": 28,
            })
        ...
        for cat, order in final_orders.items():
            ...
            if agent.trust and agent.trust.ferrari_tier_status != "internal":
                edges.append({
                    "from": agent.agent_id, "to": "dhl-logistics-01", "type": "logistics",
                    "label": f"SHIP-{cat[:3].upper()}", "message_count": 2, "status": "scheduled",
                })

        # Add compliance node
        if "eu-compliance-agent-01" not in node_ids_added:
            nodes.append({
                "id": "eu-compliance-agent-01", "label": "EU Compliance Validator", "role": "compliance_agent",
                "color": "#FF9800", "location": {"lat": 50.1109, "lon": 8.6821, "city": "Frankfurt"},
                "trust_score": 0.95, "status": "active", "size": 25,
            })
            node_ids_added.add("eu-compliance-agent-01")

        edges.append({
            "from": "ferrari-procurement-01", "to": "eu-compliance-agent-01", "type": "compliance",
            "label": "Validation", "message_count": len(qualified_agents) * 2, "status": "completed",
        })

        # Add assembly coordinator
        if "maranello-assembly-01" not in node_ids_added:
            nodes.append({
                "id": "maranello-assembly-01", "label": "Maranello Assembly", "role": "assembly_coordinator",
                "color": "#F44336", "location": {"lat": 44.5294, "lon": 10.8633, "city": "Maranello"},
                "trust_score": 1.0, "status": "active", "size": 25,
            })

        edges.append({
            "from": "ferrari-procurement-01", "to": "maranello-assembly-01", "type": "coordination",
            "label": "Assembly scheduling", "message_count": 2, "status": "confirmed",
        })
        ...
        report["dashboard"] = {
            "hero_metrics": [
                ...
                {"label": "Compliance Pass Rate", "value": f"{(checks_passed / max(report['compliance_summary']['total_checks'], 1)) * 100:.0f}%", "trend": "up"},
                {"label": "Disruptions Resolved", "value": f"{len(report['disruptions_handled'])}/{len(report['disruptions_handled'])}", "trend": None},
                ...
                {"label": "Attestations", "value": str(report.get("reputation_summary", {}).get("total_attestations", 0)), "trend": "up"},
            ],
            "cost_breakdown": [
                {"label": "Components", "value": round(total_component_cost, 2), "color": "#2196F3"},
                {"label": "Logistics", "value": round(total_logistics_cost, 2), "color": "#9C27B0"},
        ...
        report["dashboard"]["supplier_markers"].append({
            "lat": 44.5294, "lon": 10.8633, "label": "Ferrari Maranello (Assembly)",
            "type": "destination", "color": "#DC143C",
        })
        for cat, order in final_orders.items():
            agent = order["agent"]
            if agent.location and agent.location.headquarters:
                ...
                report["dashboard"]["supplier_markers"].append({
                    ...
                    "type": "supplier", "color": color_map.get(agent.role, "#2196F3"),
                })
        ...
        # Final message
        _emit("ferrari-procurement-01", "Ferrari Procurement", "system", "System", "cascade_complete",
              f"Cascade complete! Total cost: EUR {total_cost:,.0f}, {po_count} POs issued, {len(shipments)} shipments",
        ...
        _emit("system", "System", "system", "System", "error",
              f"Cascade error: {str(e)}", "", "#F44336", "error")
